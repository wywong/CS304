...
def reporttop(number=None):
    ...
    if _number==None:
        _number = "25"
    rows = TableOperation.getColumns("Borrowing AS bor INNER JOIN Book as bk ON (bor.callNumber=bk.callNumber) GROUP BY bor.callNumber LIMIT %s" % _number,['count(bor.callNumber)','bk.callNumber','bk.title'])
    session['report'] = [rows]
