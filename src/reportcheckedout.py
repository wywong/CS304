...
def reportcheckedout(subject=None):
    ...
    if _subject==None:
        ...
        duedates = [r[1]+datetime.timedelta(r[0]*7) for r in dates]
        today = date.today()
    ...
    session['reportcheckedout'] = [rows]
    return render_template('reportcheckedout.html', overdue=overdue, user=g.userInfo[0], accType=g.userInfo[8])
